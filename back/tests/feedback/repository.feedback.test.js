...
describe('Feedback Repository', () => {
  ...
  it('should add a comment on a feedback', (done) => {
    ...
    CustomerRepository
      ...
      .then(impact => impactId = impact.id)
      .then(() => FeedbackRepository.addFeedback(xebianId, impactId))
      .then(feedback => FeedbackRepository.updateFeedback(feedback.id, customerId, xebianId, impactId, 'Le module compile une fois sur deux'))
      .then(feedback => FeedbackRepository.addComment(feedback.id, xebianId, impactId, 'Ca depend d une autre équipe!', 'jsmadja@xebia.fr'))
      .then((feedback) => {
        const comment = _.pick(feedback.comments[0], ['text', 'authorEmail']);
        assert.deepEqual(comment, {
          text: 'Ca depend d une autre équipe!',
